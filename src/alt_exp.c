...
SEXP alt_exp(SEXP _A, SEXP _B, SEXP idx1, SEXP idx2, SEXP _mode, SEXP _perm, SEXP _threads, SEXP _seed)
{
    ...
    SEXP RATIO1val  = PROTECT(allocVector(REALSXP, N_feature));
    SEXP RATIO2val  = PROTECT(allocVector(REALSXP, N_feature));
...
#pragma omp parallel for num_threads(n_thread)
    for (int i = 0; i < N_feature; ++i) {
        ...
        double ratio1 = g11/g12;
        double ratio2 = g21/g22;
        // double delta = ratio1 - ratio2;
...
#pragma omp critical
        {
            ...
            REAL(Mval)[i] = mean;
            REAL(Vval)[i] = var;
            REAL(RATIO1val)[i] = ratio1;
            REAL(RATIO2val)[i] = ratio2;
        }
    }
    ...
    SET_VECTOR_ELT(ta, 4, RATIO1val);
    SET_VECTOR_ELT(ta, 5, RATIO2val);
    UNPROTECT(7);
    return ta;
