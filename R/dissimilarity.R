                         ...
                         )
{
  ...
  if (bind.assay %ni% names(object)) {
    ...
    features <- intersect(features, rownames(tab))
    tab <- tab[features,]
    bfeatures <- unique(tab[[bind.name]])
  ...
  if (length(idents) > 0 || !is.null(node)) {
    ...
    for (i in 1:length(idents)) {
      ...
      ta <- tryCatch(
        expr = {
          .Call("D_test", x[,cells], y[,cells], W[cells,cells], 1, perm, threads, idx, bidx, cs[cells], scale.factor, mode, scale, norm, seed, debug)
        },
        error = function(cond){
          return(cond$message)
        }
    ...
    if (is.null(cells)) {
      ...
      snn <- ng[['snn']]
      W <- GetWeights(snn = snn, prune.SNN = prune.SNN)
      ta <- .Call("D_test", x[,cells], y[,cells], W[cells, cells], 1, perm, threads, idx, bidx, cs[cells], scale.factor, mode, scale, norm, seed, debug)
    }
